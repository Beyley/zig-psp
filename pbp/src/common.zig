...
pub var default_file_names: [8][]const u8 = [8][]const u8{ "PARAM.SFO", "ICON0.PNG", "ICON1.PMF", "PIC0.PNG", "PIC1.PNG", "SND0.AT3", "DATA.PSP", "DATA.PSAR" };
...
pub fn readHeader(file: fs.File) !PbpHeader {
    ...
    if (bytes_read != 4 and std.mem.eql(u8, header.signature[1..], "PBP")) {
        std.debug.print("Could not read the header!", .{});
