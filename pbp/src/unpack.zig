...
pub fn unpackPBP() !void {
    ...
    var inputName = (arg_it.next() orelse {
        std.debug.print("Usage: pbptool unpack <input.pbp> <dir>\n\n", .{});
        return;
    });
    ...
    var dirName = (arg_it.next() orelse {
        std.debug.print("Usage: pbptool unpack <input.pbp> <dir>\n\n", .{});
        return;
    });
    ...
    while (i < 8) : (i += 1) {
        ...
        //Make a file!
        if (calcSize != 0) {
            std.debug.print("Creating {s} with {} bytes... ", .{ common.default_file_names[i], calcSize });

            var result = try allocator.alloc(u8, dirName.len + common.default_file_names[i].len);
            defer allocator.free(result);
            mem.copy(u8, result[0..], dirName);
            mem.copy(u8, result[dirName.len..], common.default_file_names[i]);
