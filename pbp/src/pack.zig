...
var header = common.PbpHeader{ .signature = .{ 0x00, 'P', 'B', 'P' }, .version = .{ 0x00, 0x00, 0x01, 0x00 }, .offset = .{ 40, 0, 0, 0, 0, 0, 0, 0 } };
...
pub fn packPBP() !void {
    ...
    var outputName = (arg_it.next() orelse {
    ...
    while (i < 8) : (i += 1) {
        ...
        const arg = (arg_it.next() orelse {
            ...
            std.debug.print("Found {} arguments instead of 8!\n", .{i});
    ...
    while (i < 8) : (i += 1) {
        ...
        if (filesizes[i - 1] != 0) {
            // std.debug.print("\tEntry: {s} Added at {} offset.\t\t\t(Size {} bytes)\n", .{ fileNames[i - 1], header.offset[i - 1], filesizes[i - 1] });
        }
    }
    ...
    while (i < 8) : (i += 1) {
        ...
        //Open File
        var f = try fs.cwd().openFile(fileNames[i], .{});
        defer f.close();
    ...
    // std.debug.print("Saved to {s}! (Wrote {} bytes)\n", .{ outputName, header.offset[7] + filesizes[7] });
}
