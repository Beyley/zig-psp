...
export fn sendCommandiStall(cmd: c_int, argument: c_int) void {
    ...
    if (gu_object_stack_depth == 0 and gu_curr_context == 0) {
        _ = sceGeListUpdateStallAddr(ge_list_executed[0], @as(*anyopaque, @ptrCast(gu_list.?.current)));
    }
}
...
export fn sceGuAmbient(col: u32) void {
    @setRuntimeSafety(false);
    sendCommandi(92, @as(c_int, @bitCast((col & 0xffffff))));
    sendCommandi(93, @as(c_int, @bitCast((col >> 24))));
}
...
export fn sceGuCallList(list: ?*const anyopaque) void {
    @setRuntimeSafety(false);
    var list_addr: c_int = @as(c_int, @intCast(@intFromPtr(list)));
...
export fn sceGuClutLoad(num_blocks: c_int, cbp: ?*const anyopaque) void {
    @setRuntimeSafety(false);
    var a = @as(c_int, @intCast(@intFromPtr(cbp)));
...
export fn sceGuCopyImage(psm: c_int, sx: c_int, sy: c_int, width: c_int, height: c_int, srcw: c_int, src: ?*anyopaque, dx: c_int, dy: c_int, destw: c_int, dest: ?*anyopaque) void {
    @setRuntimeSafety(false);
    var sr = @as(c_uint, @intCast(@intFromPtr(src)));
    var ds = @as(c_uint, @intCast(@intFromPtr(dest)));
...
export fn sceGuDepthRange(near: c_int, far: c_int) void {
    ...
    var max = near + far;
    var val = ((max >> 31) + max);
    var z: f32 = @as(f32, @bitCast((val >> 1)));
...
export fn sceGuDisable(state: c_int) void {
    @setRuntimeSafety(false);
    switch (@as(GuState, @enumFromInt(state))) {
        .AlphaTest => {
            sendCommandi(34, 0);
        },
        .DepthTest => {
    ...
    if ((state) < 22)
        gu_states &= @as(i32, @intCast(~(one << @as(u5, @intCast((state))))));
}
...
export fn sceGuDispBuffer(width: c_int, height: c_int, dispbp: ?*anyopaque, dispbw: c_int) void {
    ...
    if (gu_display_on != 0)
        _ = sceDisplaySetFrameBuf(@as(*anyopaque, @ptrFromInt(@intFromPtr(ge_edram_address) + @intFromPtr(gu_draw_buffer.disp_buffer))), dispbw, gu_draw_buffer.pixel_size, 1);
}

export fn sceGuDisplay(state: bool) void {
    if (state) {
        _ = sceDisplaySetFrameBuf(@as(*anyopaque, @ptrFromInt(@intFromPtr(ge_edram_address) + @intFromPtr(gu_draw_buffer.disp_buffer))), gu_draw_buffer.frame_width, gu_draw_buffer.pixel_size, 1);
    } else {
...
export fn sceGuFinish() c_int {
    switch (@as(GuContextType, @enumFromInt(gu_curr_context))) {
        .Direct, .Send => {
            sendCommandi(15, 0);
    ...
    var size: u32 = @intFromPtr(gu_list.?.current) - @intFromPtr(gu_list.?.start);
...
export fn sceGuGetStatus(state: c_int) c_int {
    if (state < 22)
        return (gu_states >> @as(u5, @intCast(state))) & 1;
    return 0;
}
...
export fn sceGuLight(light: c_int, typec: c_int, components: c_int, position: [*c]const ScePspFVector3) void {
    ...
    var lpos: usize = @as(usize, @intCast(light));
...
export fn sceGuLightColor(light: usize, component: c_int, col: c_int) void {
    @setRuntimeSafety(false);
    switch (@as(GuLightBitFlags, @enumFromInt(component))) {
        .Ambient => {
...
export fn sceGuSendList(mode: c_int, list: ?*const anyopaque, context: [*c]PspGeContext) void {
    ...
    switch (@as(GuQueueMode, @enumFromInt(mode))) {
        .Head => {
...
export fn sceGuSetCallback(signal: c_int, callback: ?*const fn (c_int) callconv(.C) void) GuCallback {
    var old_callback: GuCallback = undefined;

    switch (@as(GuCallbackId, @enumFromInt(signal))) {
        .Signal => {
...
export fn sceGuSwapBuffers() ?*anyopaque {
    ...
    if (gu_display_on != 0) {
        _ = sceDisplaySetFrameBuf(@as(*anyopaque, @ptrFromInt(@intFromPtr(ge_edram_address) + @intFromPtr(gu_draw_buffer.disp_buffer))), gu_draw_buffer.frame_width, gu_draw_buffer.pixel_size, gu_settings.swapBuffersBehaviour);
    }
...
export fn sceGuSync(mode: c_int, what: c_int) c_int {
    switch (@as(GuSyncMode, @enumFromInt(mode))) {
        .Finish => {
...
export fn sceGuTexFunc(tfx: c_int, tcc: c_int) void {
    @setRuntimeSafety(false);
    gu_contexts[@as(usize, @intCast(gu_curr_context))].texture_function = ((tcc) << 8) | (tfx);
    sendCommandi(201, (((tcc) << 8) | (tfx)) | gu_contexts[@as(usize, @intCast(gu_curr_context))].fragment_2x);
}
...
export fn sceGuTexMode(tpsm: c_int, maxmips: c_int, a2: c_int, swizzle: c_int) void {
    @setRuntimeSafety(false);
    gu_contexts[@as(usize, @intCast(gu_curr_context))].texture_mode = (tpsm);
...
export fn sceGuInit() void {
    ...
    ge_list_executed[0] = sceGeListEnQueue((@as(*anyopaque, @ptrFromInt(@intFromPtr(&ge_init_list) & 0x1fffffff))), null, gu_settings.ge_callback_id, 0);
...
export fn sceGuStart(cont: c_int, list: ?*anyopaque) void {
    @setRuntimeSafety(false);
    var local_list: [*]u32 = @as([*]u32, @ptrFromInt((@as(usize, @intCast(@intFromPtr(list))) | 0x40000000)));
    ...
    if (gu_init == 0) {
        ...
        sceGuColorMaterial(@intFromEnum(GuLightBitFlags.Ambient) | @intFromEnum(GuLightBitFlags.Diffuse) | @intFromEnum(GuLightBitFlags.Specular));
...
export fn sceGuClear(flags: c_int) void {
    @setRuntimeSafety(false);
    var context: *GuContext = &gu_contexts[@as(usize, @intCast(gu_curr_context))];
    ...
    var vertices: [*]Vertex = @as([*]Vertex, @alignCast(@ptrCast(sceGuGetMemory(@as(c_uint, @intCast(count)) * @sizeOf(Vertex)))));
    ...
    sceGuDrawArray(@intFromEnum(GuPrimitive.Sprites), @intFromEnum(VertexTypeFlags.Color8888) | @intFromEnum(VertexTypeFlags.Vertex16Bit) | @intFromEnum(VertexTypeFlags.Transform2D), @as(c_int, @intCast(count)), null, vertices);
    sendCommandi(211, 0);
}
...
export fn sceGuGetMemory(size: c_uint) *anyopaque {
    ...
    var new_ptr: [*]u32 = @as([*]u32, @ptrFromInt((@as(c_uint, @intCast(@intFromPtr(orig_ptr))) + siz + 8)));

    var lo = (8 << 24) | (@intFromPtr(new_ptr) & 0xffffff);
    var hi = (16 << 24) | ((@intFromPtr(new_ptr) >> 8) & 0xf0000);
